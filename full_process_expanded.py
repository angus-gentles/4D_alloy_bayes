...
def cont_proc():
    binaries=['GaAs','GaSb','InSb','InAs']
    data_files_names={'GaAs':'data_int_GaAs_1.csv','GaSb':'data_int_GaSb.csv','InSb':'data_int_InSb.csv','InAs':'data_int_InAs.csv'}
    data_files={}
    pbounds={}
    for i,bina in enumerate(binaries):
        data_files[bina]=np.loadtxt(data_files_names[bina],skiprows=1,delimiter=',')[:,0:2]
        pbounds[bina]=(0,len(data_files[bina])-2)
    data_file="all_data_3.csv"
    data_keys=init_keys()
